...
namespace PushUsersTask
{
    public class DeviceHandler : IDisposable
    {
        ...
        public void init(string deviceIP, int devicePort, string AppPath)
        {
        ...
        public bool GetUserInfo(int UserID, out UserInfoStruct UserInfo)
        {
        ...
        #region WorkCode
        public struct WorkCodeStruct
        {
            public int ID;
            public string Name;
        }

        public bool GetWorkCode(int WorkCodeID, ref string WorkCodeName)
        {
            WorkCodeName = "";
            if (bIsConnected == false)
            {
        ...
        public bool SetWorkCode(int WorkCodeID, string WorkCodeName)
        {
            if (bIsConnected == false)
            {
                // LOG("Please connect the device first!", "Error");
        ...
        public bool GetRecordsCount(ref int count)
        {
            if (bIsConnected == false)
            {
                // LOG("Please connect the device first!", "Error");
...
//            axCZKEM1.EnableDevice(iMachineNumber, true);//enable the device
        ...
        {
            return axCZKEM1.ClearWorkCode();
        }

        //Download the attendance records from the device(For both Black&White and TFT screen devices).
//        public bool GetDeviceRecords(ref List<AttendanceLog> records)
        public bool GetDeviceRecords(ref AttendanceLog[] records)
        {
            if (bIsConnected == false)
            {
                // LOG("Please connect the device first!", "Error");
...
//            axCZKEM1.EnableDevice(iMachineNumber, false);//disable the device
            if (axCZKEM1.ReadAttNewLogData(iMachineNumber))//read all the attendance records to the memory
            {
                    ...
                    out sdwCard, out sdwWorkcode, out idwUserMode, out idwDeviceID))//get records from the memory
                {
                    ...
                    AttendanceLog aRecord;
                    aRecord.DevIP = devIP;
                    aRecord.DateTime = idwYear.ToString().PadLeft(4, '0') +"-"+ idwMonth.ToString().PadLeft(2, '0') +"-"+ idwDay.ToString().PadLeft(2, '0')+" "
                        + idwHour.ToString().PadLeft(2, '0') +":"+ idwMinute.ToString().PadLeft(2, '0') +":"+ idwSecond.ToString().PadLeft(2, '0') + ", ";
                    aRecord.Workcode = sdwWorkcode;
                    aRecord.Usermode = idwUserMode;
                    aRecord.CardSerialNumber = sdwCard;
                    aRecord.DevID = idwDeviceID;
                    //records.Add(aRecord);
                    records[ctr] = aRecord;
                    ctr++;
                }
...
//            axCZKEM1.EnableDevice(iMachineNumber, true);//enable the device
        ...
        public bool ClearDeviceRecords()
        {
            if (bIsConnected == false)
            {
                // LOG("Please connect the device first!", "Error");
        ...
        #region Log Writer
        public void writeRecordsLog(List<string> recordList)
        {
        ...
        public void writeRecordLine(string dataRecord)
        {
            string path = ApplicationPath + "\\RecordsTemp";
            string filepath = path + "\\AttRecord-" + DateTime.Now.ToString("yyyyMMdd") + ".txt";
            if (!System.IO.Directory.Exists(path)) System.IO.Directory.CreateDirectory(path);
        ...
        public void writeLogLine(string dataLog)
        {
